...
def predict(request):
    ...
    taint_flow = TaintFlow.objects.get_or_create(hash=slice_json["flowHash"],
...
def label(request):
    ...
    project = Project.objects.get_or_create(name=label_json["project"])[0]
    taint_flow = TaintFlow.objects.get(hash=label_json["flowHash"], project=project)
    label = Label.objects.get_or_create(taint_flow=taint_flow)[0]
