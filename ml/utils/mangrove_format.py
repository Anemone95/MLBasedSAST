...
def transform(data_dir, label_dir):
    ...
    with open(_settings.relative_path_from_root('data/mangrove/t-train.txt'), 'w') as f:
        for idx in range(len(train[0])):
            label = 'truepositive' if train[1][idx] == 0 else "falsepositive"
            _slice = tokenizer.decode(train[0][idx])
            f.write('{} :: {}\n'.format(_slice, label))

    with open(_settings.relative_path_from_root('data/mangrove/t-test.txt'), 'w') as f:
        for idx in range(len(test[0])):
            label = 'truepositive' if train[1][idx] == 0 else "falsepositive"
            _slice = tokenizer.decode(train[0][idx])
            f.write('{} :: {}\n'.format(_slice, label))

    with open(_settings.relative_path_from_root('data/mangrove/dict.txt'), 'w') as f:
        for token, _int in tokenizer.get_token_dict().items():
            f.write('{} {}\n'.format(token, _int))

if __name__ == '__main__':
    transform(_settings.relative_path_from_root('data/slice/benchmark1.2'),
              _settings.relative_path_from_root('data/label/benchmark1.2'))
