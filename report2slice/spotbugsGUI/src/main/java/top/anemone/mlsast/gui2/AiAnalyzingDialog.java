...
public final class AiAnalyzingDialog extends FBDialog implements FindBugsProgress {
        ...
        AnalysisCallback callback = new AnalysisCallback() {
            ...
            public void analysisFinished(BugCollection results) {
                ...
                try {
                    ...
                    Logger.getLogger(AiAnalyzingDialog.class.getName()).log(Level.FINE, "", e);
                }
            }
     ...
     * @param callback   contains what to do if the analysis is interrupted and what to
     *                   do if it finishes normally
     * @param joinThread Whether or not this constructor should return before the
    ...
    public static void show(@Nonnull Project project, AnalysisCallback callback, boolean joinThread) {
        AiAnalyzingDialog dialog = new AiAnalyzingDialog(project, callback, joinThread);
     ...
     *                   until the analysis is either finished or interrupted.
     */
    private AiAnalyzingDialog(@Nonnull Project project, AnalysisCallback callback, boolean joinThread) {
        if (project == null) {
        ...
        SwingUtilities.invokeLater(() -> {
            ...
            try {
                ...
                AiAnalyzingDialog.this.project.getGuiCallback().showMessageDialog("ERROR DURING ANALYSIS:\n\n"
    ...
    private void cancel() {
        if (!analysisFinished) {
//            analysisThread.interrupt(); // FIXME
            analysisThread.stop();
            setVisible(false);
    ...
    public void updateCount(final int count, final int goal) {
        this.count = count;
        this.goal = goal;
        SwingUtilities.invokeLater(() -> {
            progressBar.setString(count + "/" + goal);
    ...
    private class AnalysisThread extends Thread {
        ...
        public void run() {
            ...
            scheduleDialogCleanup();
            callback.analysisFinished(null);
            MainFrame.getInstance().newProject();
        }

        private void scheduleDialogCleanup() {
            SwingUtilities.invokeLater(() -> AiAnalyzingDialog.this.setVisible(false));
        }
