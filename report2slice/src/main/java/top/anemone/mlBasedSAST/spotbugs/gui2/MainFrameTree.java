...
import top.anemone.mlBasedSAST.spotbugs.gui2.FilterActivity.FilterActivityNotifier;
...
public class MainFrameTree {
    ...
    private void setLabel(BugInstance bugInstance, boolean isTP) {
        try {
            ...
            if (!project.getBugInstanceTraceMap().containsKey(bugInstance)){
                ...
                project.getBugInstanceTraceMap().put(bugInstance, traces.get(0));
            }

            Label label=new Label(MainFrame.getInstance().getProject().toString(), project.getBugInstanceTraceMap().get(bugInstance).getHash(),isTP);
