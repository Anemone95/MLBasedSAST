 ...
 */
public class AppClassHierarchy implements IClassHierarchy {
   ...
   */
  private final ClassHierarchy.MissingSuperClassHandling superClassHandling;
    ...
    try {
      ...
      while (klass != null) {
        ...
        if (klass != null && klass.getReference().getName().equals(rootTypeRef.getName())) {
          ...
          // FIXME @Anemone idea jar
          if (klass.getName().toString().endsWith("BasicPermission")){
            System.out.println(klass);
          }
          if (scope.getExclusions().contains(klass.getName().toString().substring(1))){
            throw new NoSuperclassFoundException("Super class is in blacklist");
          }
        }
      }
    } catch (NoSuperclassFoundException e) {
      if (!superClassHandling.equals(ClassHierarchy.MissingSuperClassHandling.NONE)
          && klass instanceof BytecodeClass) {
        if (superClassHandling.equals(ClassHierarchy.MissingSuperClassHandling.PHANTOM)) {
  ...
  public boolean addClass(IClass klass) {
    ...
    while (node != null) {
      ...
      try {
        ...
        assert !superClassHandling.equals(ClassHierarchy.MissingSuperClassHandling.NONE);

        if (superClassHandling.equals(ClassHierarchy.MissingSuperClassHandling.ROOT)) superclass = getRootClass();
  ...
  /* END Custom change: remember unresolved classes */
  public ClassHierarchy.MissingSuperClassHandling getSuperClassHandling() {
